...
const BlurTool = () => {
  ...
  useLayoutEffect(() => {
    ...
    const handleMouseMove = (event) => {
      ...
      if (
        !target.classList.contains("aisr-outline") &&
        !target.closest("#aisr-ui #aisr-ui *")
      ) {
    ...
    const handleMouseDown = (event) => {
      ...
      const target = event.target;
      if (target.closest("#aisr-ui, #aisr-ui *")) {
        return;
      }
    ...
    const handleElementClick = (event) => {
      ...
      event.preventDefault();
      event.stopPropagation();
      target.classList.toggle("aisr-blur");
    };
